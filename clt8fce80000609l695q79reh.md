...
datePublished: Sun Mar 17 2024 11:00:00 GMT+0000 (Coordinated Universal Time)
cuid: clt8fce80000609l695q79reh
slug: 1-of-4-scraping-website-urls
cover: https://cdn.hashnode.com/res/hashnode/image/upload/v1710799881609/326e1b78-94ef-49bd-964f-3c4e1b17c5d6.png
...
Scraping URLs | [Scraping Websites](http://solodev.app/2-of-4-scraping-website-pages) | [Making Embeddings](http://solodev.app/3-of-4-making-embeddings-with-nomic) | [Using Embeddings](https://solodev.app/4-of-4-using-embeddings-with-llms)
...
* I use the Nano text editor to create the `web-url.js` JavaScript file:
    

```bash
sudo nano web-url.js
```
...
const scrapeLinks = async (url) => {
  ...
  $('a').each((index, element) => {
    setTimeout(() => {
      ...
      fs.appendFile('web-url.txt', 'https://mariadb.com/kb/en' + link + '\n', (err) => {
        if (err) throw err;
        counter++;
...
* From the container terminal, I run `web-url.js`:
...
* When `web-url.js` completes, I clear the container terminal:
...
* I display the results from `web-url.txt` in the container terminal:
